...
def evaluate_fold(
    ...
    max_train = parameters["sampling"]["maximum_training_rows"]
    max_calibration = parameters["sampling"]["maximum_calibration_rows"]
    if quick:
        max_train = min(max_train, 6000)
        max_calibration = min(max_calibration, 2500)
    train = deterministic_sample(train, max_train, SEEDS["sampling"] + held_out_bhe)
    calibration = deterministic_sample(
        calibration, max_calibration, SEEDS["sampling"] + 1000 + held_out_bhe
    )
...
def summarize_bhe(
    ...
    eligible_monthly = monthly.loc[monthly["hours"] >= 24].copy()
...
def main() -> None:
    ...
    test_counts = data.loc[data["split"] == "test"].groupby("bhe").size()
    evaluated_bhes = sorted(test_counts.loc[test_counts >= 24].index.astype(int).tolist())
    if args.quick:
    ...
    summary["lobo_robust_candidate"] = (
        ...
        & (
            ...
            < screening["conservative_bootstrap_upper_limit_pct"]
        )
        & (
    ...
    for bhe in ablation_bhes:
        ...
        monthly_ablation = monthly_ablation.loc[monthly_ablation["hours"] >= 24]
